...
5 raidžių žodis, 6 bandymai, dienos žodis (perjungimas 00:00 pagal pasirinktą laiko strategiją)
...
Funkcijos: validateGuess, scoreGuess, isWin, pickDailyTarget, normalizeInput
...
Deterministiškai parenkamas pagal dieną:

Skaičiuojama nuo bazės datos: 2025-01-01

Laiko zona: vietinė vartotojo (perjungimas 00:00)

Indeksas: (epochDay * 31 + wordlistVersion * 9973) % solutions.length

`wordlistVersion` įtraukiamas, kad keičiantis žodynui nesusimaišytų praeities dienos.
...
2) Streak/serijų statistika

- UX: statistikos modalas su: `gamesPlayed`, `wins`, `winPercent`, `currentStreak`, `maxStreak`, bandymų pasiskirstymo histogramą, ir (opcionaliai) mini kalendorių.
- Saugojimas: `localStorage.stats` su dienos rezultatais pagal `epochDay` (pvz., `{ [epochDay]: 'win'|'loss' }`) ir agreguotomis reikšmėmis.
- Įgyvendinimas: skaičiuoti iš įrašų po kiekvieno žaidimo; migracijos suderintos su `version`.
...
- UX: perjungiklis „Hard“ antraštėje. Įjungus – privaloma naudoti anksčiau atskleistas užuominas: žalios raidės turi likti tose pačiose pozicijose, geltonos – privalo būti žodyje.
- Įgyvendinimas: prieš `validateGuess()` – papildomas patikrinimas pagal ankstesnius bandymus. Klaidos pranešimai LT.
...
Žaidimas pagal taisykles (5x6), taisyklės matomos, klaidos LT, veikia ekrano ir fizinė klaviatūra, išsaugo būseną, testai praeina, UI responsyvus, spalvos turi alternatyvą neregintiems, rollover 00:00 veikia teisingai.
